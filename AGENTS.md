...
cc-clip bridges your local Mac/Windows clipboard to a remote Linux server over SSH, so `Ctrl+V` image paste works in remote Codex and Codex CLI sessions. It uses an xclip/wl-paste shim that transparently intercepts only Codex's clipboard calls, an X11 selection owner bridge for Codex CLI which reads the clipboard via X11 directly, and an SSH notification bridge that forwards Codex hook events (stop, permission prompt, idle) back to the local machine as native notifications.

```
Codex path:
  Local Mac clipboard → pngpaste → HTTP daemon (127.0.0.1:18339) → SSH RemoteForward → xclip shim → Codex
...
Notification path:
  Codex hook → cc-clip-hook (stdin JSON) → POST /notify via tunnel → classifier → dedup → DeliveryChain → native notification
```
...
3. **shim** (`internal/shim/template.go`) — Bash script templates for xclip and wl-paste. Intercepts two specific invocation patterns Codex uses, fetches via curl through tunnel, falls back to real binary on any failure.
...
12. **classifier** (`internal/daemon/classifier.go`) — `ClassifyHookPayload()` translates Codex hook JSON (notification, stop, etc.) into a unified `NotifyEnvelope`. Maps hook types to urgency levels: `permission_prompt`=2, `idle_prompt`=1, `stop_at_end_of_turn`=0.
...
18. **Codex wrapper** (`internal/shim/claude_wrapper.go`) — Bash script installed to `~/.local/bin/Codex` on remote. Auto-injects `--settings` with Stop and Notification hooks when tunnel is alive. Falls through to real Codex binary when tunnel is down.
...
- **Codex wrapper is conditional** — Only injects `--settings` hooks when the cc-clip tunnel is reachable (health check). When the tunnel is down, passes through transparently so Codex still works normally.
...
- **Hook script is fire-and-forget** — `cc-clip-hook` always exits 0 to avoid blocking Codex. Failures are logged to a health file, not propagated.
...
The shim only intercepts these exact Codex invocations:
...
- **Empty image race condition**: The clipboard can change between the TARGETS check (returns "image") and the image fetch (returns 204 No Content). `curl -sf` treats 204 as success → shim outputs empty bytes → Codex API rejects empty base64. Guarded by `[ ! -s "$tmpfile" ]` check in `_cc_clip_fetch_binary()`.
...
- **`~/.local/bin` PATH priority**: The shim only works if `~/.local/bin` comes before `/usr/bin` in PATH. Non-interactive SSH commands may not source `.bashrc`, so the `connect` command's `which xclip` check can show the wrong result. Interactive shells (where Codex runs) typically source `.bashrc` correctly.
